...
struct dateTimeRule {           // Defines a starting date and time in any year.   
    uint8_t    month;            // Month period begins(1-12)
    uint8_t    weekday;          // Weekday period begins (sun-sat = 1-7)
    uint8_t    instance;         // Instance in month (1=1st, 2=2nd, etc : -1=last, -2=2nd last, etc)
    uint16_t   time;             // Time (in minutes) period begins
...
struct tzRule {                 // Defines a daylight saving period and adjustment
    ...
    uint16_t      adjMinutes;    // Minutes to adjust.
